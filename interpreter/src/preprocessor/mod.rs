...
use crate::{function::FunctionTemplate, station::Station, util::*, *};
...
pub fn process<'a>(src: &str) -> Result<(), Error> {
    ...
    let mut stations: Vec<Station> = station_parser::parse_stations(&char_map)?;
    debug!(3, "Found {} stations", stations.len());

    // parsing conveyor belt connections
    let output = connection_parser::parse(&char_map, &mut stations)?;
