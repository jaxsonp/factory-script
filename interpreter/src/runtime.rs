...
use crate::{
    debug,
    pallet::Pallet,
    station::{self, Station, StationData},
    *,
};
    ...
    *step_count = 0;
    ...
    'execution_loop: while !moving_pallets.is_empty() {
        ...
        for i in 0..stations.len() {
            ...
            if input.len() < stations[i].s_type.inputs {
            ...
            if stations[i].s_type == &station::types::ASSIGN {
                ...
                continue;
            } else if stations[i].s_type == &station::types::EXIT {
                debug!(2, "Exit triggered by station");
                break 'execution_loop;
            }

            match (stations[i].s_type.procedure)(input) {
                Ok(Some(p)) => {
                    debug_assert!(
                        stations[i].s_type.output == true,
                        "Unexpected pallet returned"
                    );
                    debug!(4, "    - produced: {}", p);
                    for out_bay in stations[i].out_bays.iter() {
                        moving_pallets.push((p.clone(), *out_bay));
